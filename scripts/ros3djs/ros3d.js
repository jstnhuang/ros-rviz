...
ROS3D.makeColorMaterial = function(r, g, b, a) {
  ...
  if (a <= 0.99) {
    ...
    return new THREE.MeshPhongMaterial(
        {color: color.getHex(), opacity: a, blending: THREE.NormalBlending});
  }
};
...
ROS3D.intersectPlane = function(mouseRay, planeOrigin, planeNormal) {
  ...
  intersectPoint.addVectors(mouseRay.origin,
...
ROS3D.closestAxisPoint = function(axisRay, camera, mousePos) {
  ...
  var mpRay =
      new THREE.Ray(camera.position, vector.sub(camera.position).normalize());
 ...
 *   * f (optional) - the camera's focal length (defaults to standard Kinect
 *calibration)
 ...
 *   * varianceThreshold (optional) - threshold for variance filter, used for
...
ROS3D.DepthCloud = function(options) {
  ...
  this.video.addEventListener('loadedmetadata', this.metaLoaded.bind(this),
                              false);
 ...
 *  * loader (optional) - the Collada loader to use (e.g., an instance of
 *ROS3D.COLLADA_LOADER
 *                        ROS3D.COLLADA_LOADER_2) -- defaults to
...
ROS3D.InteractiveMarker = function(options) {
  ...
  this.onServerSetPose({pose: handle.pose});
  ...
  if (handle.menuEntries.length > 0) {
    this.menu = new ROS3D.InteractiveMarkerMenu(
        {menuEntries: handle.menuEntries, menuFontSize: handle.menuFontSize});

    // forward menu select events
    this.menu.addEventListener(
        'menu-select', function(event) { that.dispatchEvent(event); });
  }
};
 ...
 */
ROS3D.InteractiveMarker.prototype.moveAxis = function(control, origAxis,
                                                      event3d) {
  if (this.dragging) {
    ...
    var axisWorld =
        axis.clone().applyQuaternion(this.dragStart.orientationWorld.clone());
    ...
    p.addVectors(this.dragStart.position,
                 axis.clone()
 ...
 * @param event3d - the event that caused this
 */
ROS3D.InteractiveMarker.prototype.movePlane = function(control, origNormal,
                                                       event3d) {
  if (this.dragging) {
    var currentControlOri = control.currentControlOri;
    ...
    var originWorld = this.dragStart.event3d.intersection.point;
    var normalWorld =
        normal.clone().applyQuaternion(this.dragStart.orientationWorld);

    // intersect mouse ray with plane
    var intersection =
        ROS3D.intersectPlane(event3d.mouseRay, originWorld, normalWorld);
...
ROS3D.InteractiveMarker.prototype.rotateAxis = function(
    control, origOrientation, event3d) {
  if (this.dragging) {
    control.updateMatrixWorld();

    var currentControlOri = control.currentControlOri;
    var orientation =
        currentControlOri.clone().multiply(origOrientation.clone());
    ...
    var orientationWorld =
        this.dragStart.orientationWorld.clone().multiply(orientation);
...
ROS3D.InteractiveMarker.prototype.startDrag = function(control, event3d) {
  if (event3d.domEvent.button === 0) {
    ...
    this.matrixWorld.decompose(this.dragStart.positionWorld,
                               this.dragStart.orientationWorld, scale);
 ...
 * @param event3d - the event that caused this
 */
ROS3D.InteractiveMarker.prototype.setOrientation = function(control,
                                                            orientation) {
...
ROS3D.InteractiveMarker.prototype.onServerSetPose = function(event) {
  if (event !== undefined) {
    ...
    if (this.dragging) {
      ...
      this.quaternion =
          new THREE.Quaternion(pose.orientation.x, pose.orientation.y,
...
/**
 * Subscribe to the given interactive marker topic. This will unsubscribe from
 *any current topics.
 *
...
ROS3D.InteractiveMarkerClient.prototype.processUpdate = function(message) {
  ...
  // erase any markers
  message.erases.forEach(function(name) { that.eraseIntMarker(name); });
  ...
  message.markers.forEach(function(msg) {
    ...
    handle.on(
        'pose', function(pose) { intMarker.onServerSetPose({pose: pose}); });

    intMarker.addEventListener('user-pose-change',
                               handle.setPoseFromClient.bind(handle));
    intMarker.addEventListener('user-mousedown',
                               handle.onMouseDown.bind(handle));
    intMarker.addEventListener('user-mouseup', handle.onMouseUp.bind(handle));
    intMarker.addEventListener('user-button-click',
                               handle.onButtonClick.bind(handle));
 ...
 */
ROS3D.InteractiveMarkerClient.prototype.eraseIntMarker = function(
    intMarkerName) {
...
ROS3D.InteractiveMarkerControl = function(options) {
  ...
  var controlOri =
      new THREE.Quaternion(message.orientation.x, message.orientation.y,
  ...
  switch (message.interaction_mode) {
    case ROS3D.INTERACTIVE_MARKER_MOVE_AXIS:
      this.addEventListener('mousemove', this.parent.moveAxis.bind(
                                             this.parent, this, controlAxis));
      this.addEventListener('touchmove', this.parent.moveAxis.bind(
                                             this.parent, this, controlAxis));
      break;
    case ROS3D.INTERACTIVE_MARKER_ROTATE_AXIS:
      this.addEventListener('mousemove', this.parent.rotateAxis.bind(
                                             this.parent, this, controlOri));
      break;
    case ROS3D.INTERACTIVE_MARKER_MOVE_PLANE:
      this.addEventListener('mousemove', this.parent.movePlane.bind(
                                             this.parent, this, controlAxis));
      break;
    case ROS3D.INTERACTIVE_MARKER_BUTTON:
      this.addEventListener('click',
                            this.parent.buttonClick.bind(this.parent, this));
      break;
    default:
      break;
  }
  ...
  if (message.interaction_mode !== ROS3D.INTERACTIVE_MARKER_NONE) {
    this.addEventListener('mousedown',
                          this.parent.startDrag.bind(this.parent, this));
    this.addEventListener('mouseup',
                          this.parent.stopDrag.bind(this.parent, this));
    this.addEventListener('contextmenu',
                          this.parent.showMenu.bind(this.parent, this));
  ...
  switch (message.orientation_mode) {
    case ROS3D.INTERACTIVE_MARKER_INHERIT:
      ...
      this.updateMatrixWorld = function(force) {
        ROS3D.InteractiveMarkerControl.prototype.updateMatrixWorld.call(that,
                                                                        force);
    ...
    case ROS3D.INTERACTIVE_MARKER_VIEW_FACING:
      ...
      this.updateMatrixWorld = function(force) {
        ...
        var cameraRot =
            new THREE.Matrix4().extractRotation(that.camera.matrixWorld);
  ...
  message.markers.forEach(function(markerMsg) {
    var addMarker = function(transformMsg) {
      var markerHelper = new ROS3D.Marker(
          {message: markerMsg, path: that.path, loader: that.loader});
      ...
      if (transformMsg !== null) {
        ...
        // we only need to set the pose once - at least, this is what RViz seems
        // to be doing, might change in the future
...
ROS3D.InteractiveMarkerHandle.prototype.subscribeTf = function() {
  ...
  if (this.message.header.stamp.secs === 0.0 &&
      this.message.header.stamp.nsecs === 0.0) {
    this.tfClient.subscribe(this.message.header.frame_id,
                            this.tfUpdate.bind(this));
  }
};
...
ROS3D.InteractiveMarkerHandle.prototype.setPoseFromClient = function(event) {
  ...
  this.sendFeedback(ROS3D.INTERACTIVE_MARKER_POSE_UPDATE, undefined, 0,
                    event.controlName);
  ...
  if (this.dragging) {
    ...
    this.timeoutHandle =
        setTimeout(this.setPoseFromClient.bind(this, event), 250);
  }
};
...
ROS3D.InteractiveMarkerHandle.prototype.onButtonClick = function(event) {
  this.sendFeedback(ROS3D.INTERACTIVE_MARKER_BUTTON_CLICK, event.clickPosition,
                    0, event.controlName);
};
...
ROS3D.InteractiveMarkerHandle.prototype.onMouseDown = function(event) {
  this.sendFeedback(ROS3D.INTERACTIVE_MARKER_MOUSE_DOWN, event.clickPosition, 0,
                    event.controlName);
  this.dragging = true;
};
...
ROS3D.InteractiveMarkerHandle.prototype.onMouseUp = function(event) {
  this.sendFeedback(ROS3D.INTERACTIVE_MARKER_MOUSE_UP, event.clickPosition, 0,
                    event.controlName);
  this.dragging = false;
  if (this.timeoutHandle) {
    clearTimeout(this.timeoutHandle);
...
ROS3D.InteractiveMarkerHandle.prototype.onMenuSelect = function(event) {
  this.sendFeedback(ROS3D.INTERACTIVE_MARKER_MENU_SELECT, undefined, event.id,
                    event.controlName);
};
 ...
 * @param clickPosition (optional) - the position in ROS space the click
 *happened
 * @param menuEntryID (optional) - the menu entry ID that is associated
 * @param controlName - the name of the control
 */
ROS3D.InteractiveMarkerHandle.prototype.sendFeedback = function(
    eventType, clickPosition, menuEntryID, controlName) {
...
ROS3D.InteractiveMarkerMenu = function(options) {
  ...
  var overlayClassName =
      options.overlayClassName || 'default-interactive-marker-overlay';
  ...
  if (document.getElementById('default-interactive-marker-menu-css') === null) {
    ...
    style.type = 'text/css';
    style.innerHTML = '.default-interactive-marker-menu {' +
                      ...
                      'z-index: 1002;' +
                      '}' +
                      '.default-interactive-marker-menu ul {' +
                      'padding: 0px 0px 5px 0px;' +
                      'margin: 0px;' +
                      'list-style-type: none;' +
                      '}' +
                      '.default-interactive-marker-menu ul li div {' +
                      ...
                      '}' +
                      '.default-interactive-marker-menu-entry:hover {' +
                      '  background-color: #666666;' +
                      '  cursor: pointer;' +
                      '}' +
                      '.default-interactive-marker-menu ul ul {' +
                      '  font-style: italic;' +
                      '  padding-left: 10px;' +
                      '}' +
                      '.default-interactive-marker-overlay {' +
                      '  position: absolute;' +
                      '  top: 0%;' +
                      '  left: 0%;' +
                      '  width: 100%;' +
  ...
  this.menuDomElem.addEventListener(
    ...
    for (var i = 0; i < children.length; i++) {
      ...
      if (children[i].children.length > 0) {
        ...
        divElem.addEventListener('click',
                                 emitMenuSelect.bind(that, children[i]));
        divElem.addEventListener('touchstart',
                                 emitMenuSelect.bind(that, children[i]));
 ...
 * @param options - object with following keys:
 *
 *   * path - the base path or URL for any mesh files that will be loaded for
 *this marker
...
ROS3D.Marker = function(options) {
  ...
  var colorMaterial = ROS3D.makeColorMaterial(this.msgColor.r, this.msgColor.g,
                                              this.msgColor.b, this.msgColor.a);
  ...
  switch (message.type) {
    case ROS3D.MARKER_ARROW:
      ...
      if (message.points.length === 2) {
        p1 = new THREE.Vector3(message.points[0].x, message.points[0].y,
                               message.points[0].z);
        var p2 = new THREE.Vector3(message.points[1].x, message.points[1].y,
                                   message.points[1].z);
    ...
    case ROS3D.MARKER_CUBE:
      ...
      var cubeGeom = new THREE.CubeGeometry(message.scale.x, message.scale.y,
                                            message.scale.z);
    ...
    case ROS3D.MARKER_CYLINDER:
      ...
      cylinderMesh.quaternion.setFromAxisAngle(new THREE.Vector3(1, 0, 0),
                                               Math.PI * 0.5);
      cylinderMesh.scale =
          new THREE.Vector3(message.scale.x, message.scale.z, message.scale.y);
      this.add(cylinderMesh);
      break;
    case ROS3D.MARKER_LINE_STRIP:
      var lineStripGeom = new THREE.Geometry();
      var lineStripMaterial =
          new THREE.LineBasicMaterial({size: message.scale.x});
      ...
      if (message.colors.length === message.points.length) {
        ...
        for (j = 0; j < message.points.length; j++) {
          ...
          clr.setRGB(message.colors[j].r, message.colors[j].g,
                     message.colors[j].b);
          lineStripGeom.colors.push(clr);
        }
      } else {
        lineStripMaterial.color.setRGB(message.color.r, message.color.g,
                                       message.color.b);
      }
    ...
    case ROS3D.MARKER_LINE_LIST:
      ...
      if (message.colors.length === message.points.length) {
        ...
        for (k = 0; k < message.points.length; k++) {
          ...
          c.setRGB(message.colors[k].r, message.colors[k].g,
                   message.colors[k].b);
          lineListGeom.colors.push(c);
        }
      } else {
        lineListMaterial.color.setRGB(message.color.r, message.color.g,
                                      message.color.b);
      }

      // add the line
      this.add(
          new THREE.Line(lineListGeom, lineListMaterial, THREE.LinePieces));
      break;
    case ROS3D.MARKER_CUBE_LIST:
      ...
      for (p = 0; p < numPoints; p += stepSize) {
        cube = new THREE.CubeGeometry(message.scale.x, message.scale.y,
                                      message.scale.z);

        // check the color
        if (createColors) {
          curColor =
              ROS3D.makeColorMaterial(message.colors[p].r, message.colors[p].g,
                                      message.colors[p].b, message.colors[p].a);
        } else {
    ...
    case ROS3D.MARKER_POINTS:
      ...
      var material = new THREE.ParticleBasicMaterial({size: message.scale.x});
      ...
      if (message.colors.length === message.points.length) {
        ...
        for (i = 0; i < message.points.length; i++) {
          var color = new THREE.Color();
          color.setRGB(message.colors[i].r, message.colors[i].g,
                       message.colors[i].b);
          geometry.colors.push(color);
        }
      } else {
        material.color.setRGB(message.color.r, message.color.g,
                              message.color.b);
      }
    ...
    case ROS3D.MARKER_TRIANGLE_LIST:
      ...
      tri.scale =
          new THREE.Vector3(message.scale.x, message.scale.y, message.scale.z);
      this.add(tri);
      break;
    default:
...
ROS3D.Marker.prototype.update = function(message) {
  ...
  if (message.color.r !== this.msgColor.r ||
    ...
    var colorMaterial = ROS3D.makeColorMaterial(
        message.color.r, message.color.g, message.color.b, message.color.a);
    ...
    switch (message.type) {
      ...
      case ROS3D.MARKER_CUBE_LIST:
      case ROS3D.MARKER_SPHERE_LIST:
        // TODO Support to update color for MARKER_CUBE_LIST &
        // MARKER_SPHERE_LIST
        return false;
      default:
        return false;
  ...
  // Update geometry
  var scaleChanged = Math.abs(this.msgScale[0] - message.scale.x) > 1.0e-6 ||
      ...
      if (message.action === 0) {
        ...
        if (!updated) {  // "ADD"
          var newMarker = new ROS3D.Marker(
              {message: message, path: that.path, loader: that.loader});
        ...
        console.warn(
            'Received marker message with unknown action identifier "' +
            message.action + '"');
      }
    });
...
ROS3D.Arrow = function(options) {
  ...
  var geometry = new THREE.CylinderGeometry(
      shaftDiameter * 0.5, shaftDiameter * 0.5, shaftLength, 12, 1);
  ...
  // create the head
  var coneGeometry =
      new THREE.CylinderGeometry(0, headDiameter * 0.5, headLength, 12, 1);
...
ROS3D.Arrow.prototype.setDirection = function(direction) {
  ...
  var radians =
      Math.acos(new THREE.Vector3(0, 1, 0).dot(direction.clone().normalize()));
...
/**
 * An Axes object can be used to display the axis of a particular coordinate
 *frame.
 *
 * @constructor
 * @param options - object with following keys:
...
ROS3D.Axes = function(options) {
  ...
  this.lineGeom =
      new THREE.CylinderGeometry(shaftRadius, shaftRadius, 1.0 - headLength);
...
ROS3D.Grid = function(options) {
  ...
  for (var i = 0; i <= num_cells; ++i) {
    ...
    geometryH.vertices.push(new THREE.Vector3(-edge, position, 0),
                            new THREE.Vector3(edge, position, 0));
    var geometryV = new THREE.Geometry();
    geometryV.vertices.push(new THREE.Vector3(position, -edge, 0),
 ...
 * @param options - object with following keys:
 *
 *  * path (optional) - the base path to the associated models that will be
 *loaded
    ...
    loader = new THREE.STLLoader();
    {
      loader.load(uri, function(geometry) {
        ...
        if (material !== null) {
          ...
          mesh = new THREE.Mesh(
              geometry, new THREE.MeshBasicMaterial({color: 0x999999}));
        }
        that.add(mesh);
      });
    }
...
/**
 * A TriangleList is a THREE object that can be used to display a list of
 *triangles as a geometry.
 *
 * @constructor
 * @param options - object with following keys:
 *
...
/**
 * An OccupancyGrid can convert a ROS occupancy grid message into a THREE
 *object.
 *
 * @constructor
 * @param options - object with following keys:
...
ROS3D.OccupancyGrid = function(options) {
  ...
  // move the map so the corner is at X, Y and correct orientation (informations
  // from message.info)
  this.useQuaternion = true;
  this.quaternion = message.info.origin.orientation;
  this.position.x =
      (width * message.info.resolution) / 2 + message.info.origin.position.x;
  this.position.y =
      (height * message.info.resolution) / 2 + message.info.origin.position.y;
 ...
 *   * continuous (optional) - if the map should be continuously loaded (e.g.,
 *for SLAM)
...
ROS3D.OccupancyGridClient = function(options) {
  ...
  rosTopic.subscribe(function(message) {
    ...
    var newGrid = new ROS3D.OccupancyGrid({message: message});
...
ROS3D.Odometry = function(options) {
  ...
  var rosTopic = new ROSLIB.Topic(
      {ros: ros, name: topic, messageType: 'nav_msgs/Odometry'});
  ...
  rosTopic.subscribe(function(message) {
    ...
    that.options.origin = new THREE.Vector3(message.pose.pose.position.x,
...
/**
 * A Path client that listens to a given topic and displays a line connecting
 *the poses.
 *
 * @constructor
 * @param options - object with following keys:
...
ROS3D.Path = function(options) {
  ...
  var rosTopic =
      new ROSLIB.Topic({ros: ros, name: topic, messageType: 'nav_msgs/Path'});
  ...
  rosTopic.subscribe(function(message) {
    ...
    for (var i = 0; i < message.poses.length; i++) {
      var v3 = new THREE.Vector3(message.poses[i].pose.position.x,
...
ROS3D.Point = function(options) {
  ...
  var rosTopic = new ROSLIB.Topic(
      {ros: ros, name: topic, messageType: 'geometry_msgs/PointStamped'});
...
/**
 * A PolygonStamped client that listens to a given topic and displays the
 *polygon
 *
 * @constructor
 * @param options - object with following keys:
...
ROS3D.Polygon = function(options) {
  ...
  var rosTopic = new ROSLIB.Topic(
      {ros: ros, name: topic, messageType: 'geometry_msgs/PolygonStamped'});
  ...
  rosTopic.subscribe(function(message) {
    ...
    for (var i = 0; i < message.polygon.points.length; i++) {
      v3 = new THREE.Vector3(message.polygon.points[i].x,
                             message.polygon.points[i].y,
                             message.polygon.points[i].z);
      lineGeometry.vertices.push(v3);
    }
    v3 = new THREE.Vector3(message.polygon.points[0].x,
                           message.polygon.points[0].y,
...
ROS3D.Pose = function(options) {
  ...
  var rosTopic = new ROSLIB.Topic(
      {ros: ros, name: topic, messageType: 'geometry_msgs/PoseStamped'});
  ...
  rosTopic.subscribe(function(message) {
    ...
    that.options.origin =
        new THREE.Vector3(message.pose.position.x, message.pose.position.y,
...
ROS3D.PoseArray = function(options) {
  ...
  var rosTopic = new ROSLIB.Topic(
      {ros: ros, name: topic, messageType: 'geometry_msgs/PoseArray'});
  ...
  rosTopic.subscribe(function(message) {
    ...
    for (var i = 0; i < message.poses.length; i++) {
      ...
      var v3 = new THREE.Vector3(message.poses[i].position.x,
                                 message.poses[i].position.y,
                                 message.poses[i].position.z);
      lineGeometry.vertices.push(v3);

      var rot = new THREE.Quaternion(
          message.poses[i].orientation.x, message.poses[i].orientation.y,
 ...
 *  * texture - (optional) Image url for a texture to use for the points.
 *Defaults to a single white pixel.
 *  * rootObject (optional) - the root object to add this marker to
...
ROS3D.LaserScan = function(options) {
  ...
  var rosTopic = new ROSLIB.Topic(
      {ros: ros, name: topic, messageType: 'sensor_msgs/LaserScan'});
  ...
  rosTopic.subscribe(function(message) {
    ...
    for (var i = 0; i < n; i++) {
      ...
      if (range < message.range_min || range > message.range_max) {
        ...
        that.particles.points[i] = new THREE.Vector3(
            range * Math.cos(angle), range * Math.sin(angle), 0.0);
...
ROS3D.Particles = function(options) {
  ...
  var texture =
      options.texture ||
      'https://upload.wikimedia.org/wikipedia/commons/a/a2/Pixel-white.png';
...
function setFrame(particles, frame) {
  if (particles.sn === null) {
    particles.sn = new ROS3D.SceneNode(
        {frameID: frame, tfClient: particles.tfClient, object: particles.ps});
...
ROS3D.PointCloud2 = function(options) {
  ...
  var first_cb = options.first_cb || function() {};
  var that = this;

  this.particles = new ROS3D.Particles(options);

  this.rosTopic = new ROSLIB.Topic(
      {ros: ros, name: topic, messageType: 'sensor_msgs/PointCloud2'});

  var first = false;

  this.handlePointCloud = function(message) {
    if (!first) {
      first = true;
      first_cb && first_cb();
    }
...
/**
 * A URDF can be used to load a ROSLIB.UrdfModel and its associated models into
 *a 3D object.
 *
 * @constructor
 * @param options - object with following keys:
 ...
 *   * tfClient - the TF client handle to use
 *   * path (optional) - the base path to the associated Collada models that
 *will be loaded
    ...
    for (var i = 0; i < link.visuals.length; i++) {
      ...
      if (visual && visual.geometry) {
        ...
        if (visual.material && visual.material.color) {
          ...
          colorMaterial =
              ROS3D.makeColorMaterial(color.r, color.g, color.b, color.a);
        }
        if (visual.geometry.type === ROSLIB.URDF_MESH) {
          ...
          if (fileType === '.dae' || fileType === '.stl') {
            ...
            if (link.visuals[i].geometry.scale) {
              mesh.scale = new THREE.Vector3(visual.geometry.scale.x,
          ...
          switch (visual.geometry.type) {
            case ROSLIB.URDF_BOX:
              ...
              var cube =
                  new THREE.CubeGeometry(dimension.x, dimension.y, dimension.z);
            ...
            case ROSLIB.URDF_CYLINDER:
              ...
              shapeMesh.quaternion.setFromAxisAngle(new THREE.Vector3(1, 0, 0),
                                                    Math.PI * 0.5);
              break;
...
ROS3D.Urdf.prototype.updateTfPrefix =
    function(tfPrefix) {
    ...
    /**
     * A URDF client can be used to load a URDF and its associated models into a
     *3D object from the ROS
     * parameter server.
     *
     * Emits the following events:
     *
     * * 'change' - emited after the URDF and its meshes have been loaded into
     *the root object
     *
     * @constructor
     * @param options - object with following keys:
     *
     *   * ros - the ROSLIB.Ros connection handle
     *   * param (optional) - the paramter to load the URDF from, like
     *'robot_description'
     *   * tfClient - the TF client handle to use
     *   * path (optional) - the base path to the associated Collada models that
     *will be loaded
    ...
    ROS3D.UrdfClient = function(options) {
      ...
      var getParam = new ROSLIB.Param({ros: ros, name: param});
      getParam.get(function(string) {
        // hand off the XML string to the URDF model
        var urdfModel = new ROSLIB.UrdfModel({string: string});
...
/**
 * A SceneNode can be used to keep track of a 3D object with respect to a ROS
 *frame within a scene.
 *
 * @constructor
 * @param options - object with following keys:
 *
...
ROS3D.Viewer = function(options) {
  ...
  this.cameraControls =
      new ROS3D.OrbitControls({scene: this.scene, camera: this.camera});
  ...
  this.highlighter = new ROS3D.Highlighter({mouseHandler: mouseHandler});
  ...
  function draw() {
    ...
    that.directionalLight.position =
        that.camera.localToWorld(new THREE.Vector3(-1, 1, 0));
 ...
 * @param selectable (optional) - if the object should be added to the
 *selectable list
 */
...
/**
 * Add all corresponding webgl objects in the given scene and add them to the
 *given render list.
 *
 ...
 */
ROS3D.Highlighter.prototype.getWebglObjects = function(scene, objects,
                                                       renderList) {
 ...
 */
ROS3D.Highlighter.prototype.renderHighlight = function(renderer, scene,
                                                       camera) {
  // get webgl objects
  var renderList = [];
...
/**
 * Process the particular DOM even that has occurred based on the mouse's
...
ROS3D.MouseHandler.prototype.processDomEvent = function(domEvent) {
  ...
  var mouseRaycaster =
      new THREE.Raycaster(this.camera.position.clone(),
  ...
  if (this.dragging) {
    ...
    if ((domEvent.type === 'mouseup' && domEvent.button === 2) ||
        domEvent.type === 'click' || domEvent.type === 'touchend') {
      this.dragging = false;
    }
    return;
  }
  ...
  // if the mouse moves from one object to another (or from/to the 'null'
  // object), notify both
  ...
  // if the finger moves from one object to another (or from/to the 'null'
  // object), notify both
...
ROS3D.MouseHandler.prototype.notify = function(target, type, event3D) {
  ...
  event3D.stopPropagation = function() { event3D.cancelBubble = true; };
  ...
  while (event3D.currentTarget) {
    ...
    if (event3D.currentTarget.dispatchEvent &&
            event3D.currentTarget.dispatchEvent instanceof
            Function) {
...
/**
 * Behaves like THREE.OrbitControls, but uses right-handed coordinates and z as
 *up vector.
 *
 * @constructor
...
ROS3D.OrbitControls = function(options) {
  ...
  this.axes =
      new ROS3D.Axes({shaftRadius: 0.025, headRadius: 0.07, headLength: 0.2});
  ...
  function onMouseDown(event3D) {
    ...
    switch (event.button) {
      ...
      case 1:
        ...
        moveStartIntersection = intersectViewPlane(
            event3D.mouseRay, moveStartCenter, moveStartNormal);
        break;
      case 2:
        state = STATE.ZOOM;
  ...
  function onMouseMove(event3D) {
    ...
    if (state === STATE.ROTATE) {
      ...
      that.rotateLeft(2 * Math.PI * rotateDelta.x / pixelsPerRound *
                      that.userRotateSpeed);
      that.rotateUp(2 * Math.PI * rotateDelta.y / pixelsPerRound *
                    that.userRotateSpeed);
      ...
      var intersection =
          intersectViewPlane(event3D.mouseRay, that.center, moveStartNormal);
    ...
    switch (event.touches.length) {
      ...
      case 2:
        ...
        touchStartPosition[0] =
            new THREE.Vector2(event.touches[0].pageX, event.touches[0].pageY);
        touchStartPosition[1] =
            new THREE.Vector2(event.touches[1].pageX, event.touches[1].pageY);
  ...
  function onTouchMove(event3D) {
    var event = event3D.domEvent;
    if (state === STATE.ROTATE) {
      rotateEnd.set(event.touches[0].pageX - window.scrollX,
                    event.touches[0].pageY - window.scrollY);
      rotateDelta.subVectors(rotateEnd, rotateStart);
      ...
      if (state === STATE.MOVE) {
        ...
        var delta = new THREE.Vector3().subVectors(
            moveStartIntersection.clone(), intersection.clone());
        that.center.addVectors(moveStartCenter.clone(), delta.clone());
        that.camera.position.addVectors(moveStartPosition.clone(),
                                        delta.clone());
        that.update();
        that.camera.updateMatrixWorld();
      }
...
ROS3D.OrbitControls.prototype.showAxes = function() {
  ...
  this.axes.traverse(function(obj) { obj.visible = true; });
  if (this.hideTimeout) {
    clearTimeout(this.hideTimeout);
  }
  this.hideTimeout = setTimeout(function() {
    that.axes.traverse(function(obj) { obj.visible = false; });
...
ROS3D.OrbitControls.prototype.update = function() {
  ...
  // angle from y-axis
  var phi = Math.atan2(Math.sqrt(offset.y * offset.y + offset.x * offset.x),
                       offset.z);
